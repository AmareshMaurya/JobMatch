...
export const postApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  console.log("Resume file received:", resume); // Log the resume file
  ...
  console.log("Uploading resume to Cloudinary..."); // Log before Cloudinary upload
  const cloudinaryResponse = await cloudinary.uploader.upload(resume.tempFilePath);
  if (!cloudinaryResponse || cloudinaryResponse.error) {
    console.error("Cloudinary Error:", cloudinaryResponse.error || "Unknown Cloudinary error");
    return next(new ErrorHandler("Failed to upload Resume to Cloudinary", 500));
  }
  console.log("Resume uploaded successfully:", cloudinaryResponse.secure_url); // Log successful upload

  const { name, email, coverLetter, phone, address, jobId } = req.body;
  console.log("Request body:", { name, email, coverLetter, phone, address, jobId }); // Log the request body
...
// Modify the employerGetAllApplications controller
export const employerGetAllApplications = catchAsyncErrors(async (req, res, next) => {
  ...
  const { _id } = req.user;
  console.log("Fetching applications for employer:", _id); // Log employer ID
  const applications = await Application.find({ "employerID.user": _id }).populate("jobId");
  console.log("Applications found:", applications); // Log the fetched applications
  res.status(200).json({
    success: true,
    applications,
  });
